...
"""High-level inference runtime for dadvar System 1 decision models."""
import json
import os
import warnings
...
class Agent:
    ...
    def __init__(
        ...
        if rejected:
            warnings.warn(
                "dadvar: this checkpoint ships temperatures outside [%g, %g] which would distort "
        ...
        if fell_back_from is not None:
            print(
                "\n[dadvar] Warning: could not place the model on %s, so it is running on CPU.\n"
                "  Reason: %s\n"
...
def load(model_id_or_path: str = "convaiinnovations/laya", device: Optional[str] = None,
    ...
    `subfolder` picks one checkpoint out of a repo that bundles several:
        ...
        dadvar.load("convaiinnovations/laya")                           # English (repo root)
        dadvar.load("convaiinnovations/laya", subfolder="multilingual")
    """
